...
const Ω_base = 2π * 50e6     # Base single-photon Rabi frequency (50 MHz)
const Ω_π    = Ω_base / √2   # π-polarization strength
const Ω_σ    = Ω_base / 2    # σ±-polarization strength

const Δ      = -2π * 1.85e9  # One-photon detuning from ³D₁
const A_hfs  = 2π * 4.5e9    # Hyperfine splitting between excited manifolds
const Γ_3D1  = 2π * 6.9e6    # ³D₁ natural linewidth (~23 ns lifetime)
const η_leak = 0.36          # Branching to ³P₁,₂ (leakage fraction)
const B      = 0.0G          # Magnetic field in Gauss

const dt      = 0.005e-9
const T_total = 1.31e-6    # Total evolution time (s)
